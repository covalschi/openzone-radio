...
modded class TransmitterBase
{
    ...
    override void CF_OnStoreSave(CF_ModStorageMap storage)
    {
        ...
        // Ефір НЕ зберігається, і це рішення, а не пропуск.
        //
        // Спробували 2026-09-02 і прибрали того ж дня. Задум був у тому, щоб
        // підкинута відкрита рація пережила рестарт; виявилось, що майбутнім
        // «жучкам» це не потрібне взагалі -- вони вмикатимуть ефір на кожному
        // OnWorkStart за своїм прапорцем у профілі, без жодної пам'яті.
        // Лишався б єдиний випадок -- забута із замком рація повертається
        // гарячим мікрофоном, -- а це радше вада, ніж властивість.
        //
        // Дорогою намірялось інше й корисне: рація повертається ВИМКНЕНОЮ
        // після перезаходу (у лозі стояло "no power (switched off)"), тож
        // будь-яке відновлення ефіру довелось би починати з вмикання. Це
        // питання жучків, і вирішувати його треба там.
    }
