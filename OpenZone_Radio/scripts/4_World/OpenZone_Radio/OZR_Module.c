...
class OZR_Module : CF_ModuleWorld
{
    ...
    private TransmitterBase OZR_PickSpeaker(PlayerBase player, array<EntityAI> items)
    {
        ...
        for (int i = 0; i < items.Count(); i++)
        {
            ...
            // РАЦІЯ, ЯКОЇ НЕ ВИБИРАЛИ, НЕ ГОВОРИТЬ. Рішення власника
            // 2026-09-02: воно замінило запасний хід «за відсутності пам'яті
            // говорить перша ж», де за гравця мовчки вибирав порядок обходу
            // інвентаря -- між рaціями на різних частотах.
            //
            // Вибором рахується взяття в руки, і мітка від нього переживає
            // вихід і рестарт (CF_ModStorage), тож жест потрібен один раз на
            // рацію, а не щосесії.
            //
            // Рук це не стосується: перевірка рук стоїть ВИЩЕ за цю, і рація
            // в долоні говорить навіть тоді, коли мітки на ній ще немає.
            if (!t.OZR_Chosen())
                continue;
