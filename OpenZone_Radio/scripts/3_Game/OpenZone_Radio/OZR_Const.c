...
class OZR_Const
{
    ...
    // Ім'я КЛАСУ в CfgMods, і воно НЕ те саме, що MOD вище.
    //
    // MOD -- простір імен RPC, спільний для родини. А CF_ModStorage ключує
    // свою карту саме іменем класу (modloader.c: _CF_Init(i, name, ...),
    // cf_modstoragemap.c: Insert(mod.GetName(), storage)), і по чужому ключу
    // Get() поверне порожньо -- тобто збереження мовчки не працюватиме, без
    // жодної помилки. Класична пастка ModStorage, і саме тому це окремий
    // рядок із поясненням, а не MOD, який «схожий».
    //
    // ЗМІНА ЦЬОГО РЯДКА = ЗМІНА ФОРМАТУ ЗБЕРЕЖЕННЯ. CF мітить блок парою
    // хешів цього імені; інше ім'я -- інші хеші, і всі збережені дані стають
    // недосяжними (байти цілі, але їх ніхто не забере).
    static const string MOD_CLASS = "OpenZone_Radio";
