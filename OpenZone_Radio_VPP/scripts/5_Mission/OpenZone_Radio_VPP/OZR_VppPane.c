...
// крок -- найбільший спільний дільник усіх кроків i всіх зміщень меж. Тому
// профіль не може «не влізти в сітку»: сітка йде за ним.
//
// Але два наслідки цього в тексті JSON не видно взагалі, i саме заради них
// панель існує:
//
...
//   2. Є СТЕЛЯ, i вона не з рушія, а з рішення власника (2026-09-01, ТЗ-5
//      R-E4): 65536 ділень ефіру, i крапка. Ефір, якому треба більше, сервер
...
//
// I третє, вже про час: файл сітки читає нативний патч при старті ПРОЦЕСУ, тож
...
//
// Дальність НЕ РЕДАГУЄТЬСЯ, i це не лінощі. `range` живе в CfgVehicles класу,
// його читає сам рушій, а скрипту доступні лише ConfigGetFloat i рідня --
// жодного ConfigSet у грі немає. Змінити дальність на живому сервері
// неможливо; вона змінюється в config.cpp i переупаковкою pbo. Тому поле
// показує справжнє число з конфігу класу i мовчить: поле вводу, яке нічого не
...
modded class OZ_VppAdminMenu
{
    ...
    // й на сервері, i з тих самих профілів -- інакше попередження тут i
    ...
    private void PaintChecks()
    {
        ...
        // Друге питання, i єдине, яке ще може мати відповідь «ні»: чи працює
    ...
    private void NewRad()
    {
        ...
        // того, як їй дали ім'я, i вона працюватиме.
        if (OZR_ClientGrid.Ready())
        {
    ...
    private void SaveRad()
    {
        ...
        // Поля належать ВИБРАНОМУ рядку, i лише йому: решта вже в об'єкті.
        if (m_RadPicked >= 0 && m_RadPicked < m_RadCfg.Radios.Count())
        {
